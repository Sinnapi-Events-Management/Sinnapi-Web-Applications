  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from('vendor_applications')
        .select(
          'id,business_name,status,is_reapplication,submitted_at,created_at,profiles:applicant_id(full_name,email)',
        )
        .order('created_at', { ascending: false });
      return (data ?? []) as ApplicationListModel[];
    },
  });
}
...
export function useApplication(id: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from('vendor_applications')
        .select('*,profiles:applicant_id(full_name,email,phone)')
        .eq('id', id)
        .maybeSingle();
      return (data as ApplicationDetailModel) ?? null;
    },
  });
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .order('created_at', { ascending: false });
      return (data ?? []) as VendorAdminModel[];
    },
  });
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .limit(200);
      return (data ?? []) as LedgerEntryModel[];
    },
  });
}
...
export function useEventsAdmin() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from('events')
        .select('id,title,source,status,event_date,is_public,created_at')
        .order('created_at', { ascending: false });
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select('id,trigger_key,channel,subject,locale,is_active')
        .order('trigger_key');
      return (data ?? []) as NotificationTemplateModel[];
    },
  });
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .order('data_category');
      return (data ?? []) as RetentionPolicyModel[];
    },
  });
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .order('created_at', { ascending: false });
      return (data ?? []) as ErasureRequestModel[];
    },
  });
}

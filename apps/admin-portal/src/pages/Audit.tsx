      ...
      <QueryState isLoading={isLoading} error={error}>
        {rows.length === 0 ? <EmptyState title="No audit entries" /> : (
          <Card variant="outlined" sx={{ overflowX: "auto" }}>
            <Table size="small">
              <TableHead><TableRow><TableCell>When</TableCell><TableCell>Action</TableCell><TableCell>Entity</TableCell><TableCell>Actor</TableCell></TableRow></TableHead>
              <TableBody>
                {rows.map((l: any) => (
                  <TableRow key={l.id} hover>
                    <TableCell>{formatDateTime(l.occurred_at)}</TableCell>
                    <TableCell><Typography variant="body2" fontWeight={600}>{l.action}</Typography></TableCell>
                    <TableCell>{l.entity_type}{l.entity_id ? ` · ${String(l.entity_id).slice(0, 8)}` : ""}</TableCell>

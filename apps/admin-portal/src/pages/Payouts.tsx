...
import { Card, Table, TableHead, TableRow, TableCell, TableBody, Button, Alert, Stack } from "@mui/material";
...
export default function Payouts() {
  ...
  function refresh() { qc.invalidateQueries({ queryKey: ["admin-payouts"] }); }
  ...
  async function process(id: string) {
    ...
    setBusy(null);
    if (!res.ok) { setErr((await res.json().catch(() => ({})))?.error ?? "Payout failed"); return; }
    refresh();
  }

  return (
    <>
      <PageTitle title="Payouts" subtitle="Approve (Finance) then process disbursement. Approver must differ from requester." />
        ...
        {rows.length === 0 ? (
          ...
          <Card variant="outlined">
            <Table>
              <TableHead>
                <TableRow>
                  <TableCell>Vendor</TableCell><TableCell align="right">Amount</TableCell><TableCell>Requested</TableCell>
              ...
              <TableBody>
                {rows.map((p: any) => (
                  <TableRow key={p.id} hover>
                    <TableCell>{one<any>(p.vendors)?.business_name ?? "—"}</TableCell>
                    ...
                    <TableCell align="right">
                      <Stack direction="row" spacing={1} justifyContent="flex-end">
                        {has("payout.approve") && p.status === "requested" && (
                          <Button size="small" variant="contained" disabled={busy === p.id} onClick={() => approve(p.id)}>Approve</Button>
                        )}
                        {has("payout.process") && p.status === "approved" && (
                          <Button size="small" variant="outlined" disabled={busy === p.id} onClick={() => process(p.id)}>Process</Button>
                        )}
                      </Stack>
                    </TableCell>
                  </TableRow>

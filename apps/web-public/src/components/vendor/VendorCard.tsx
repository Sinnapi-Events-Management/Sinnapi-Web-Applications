...
import { Card, CardActionArea, CardContent, CardMedia, Box, Typography, Chip, Rating, Stack } from "@mui/material";
...
export default function VendorCard({ vendor }: { vendor: VendorCardModel }) {
  ...
  return (
    <Card variant="outlined" sx={{ height: "100%", transition: "box-shadow .2s", "&:hover": { boxShadow: 4 } }}>
      <CardActionArea component={NextLink} href={`/vendors/${vendor.slug}`} sx={{ height: "100%" }}>
        <Box sx={{ position: "relative" }}>
          <CardMedia component="img" height="180" image={image} alt={vendor.business_name} sx={{ bgcolor: "grey.100" }} />
          {vendor.is_featured && (
            <Chip size="small" color="secondary" label="Featured" sx={{ position: "absolute", top: 12, left: 12 }} />
          )}
        </Box>
        <CardContent>
          ...
          {price && (
            <Typography variant="body2" sx={{ mt: 1 }}>
              ...
              <Box component="span" fontWeight={600}>{price}</Box>
            </Typography>
          )}
        </CardContent>
      </CardActionArea>

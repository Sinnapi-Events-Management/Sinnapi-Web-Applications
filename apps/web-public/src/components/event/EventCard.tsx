  ...
  return (
    <Card variant="outlined" sx={{ height: "100%" }}>
      <CardActionArea component={NextLink} href={`/events/${event.id}`} sx={{ height: "100%" }}>
        <CardMedia component="img" height="160" image={event.cover_image_url ?? "/placeholder-event.svg"} alt={event.title} sx={{ bgcolor: "grey.100" }} />
        <CardContent>
          <Stack direction="row" spacing={1} sx={{ mb: 1 }}>
            {event.event_type && <Chip size="small" label={titleize(event.event_type)} />}
            <Chip size="small" variant="outlined" label={event.source === "admin" ? "Inspiration" : "Open event"} />
          </Stack>

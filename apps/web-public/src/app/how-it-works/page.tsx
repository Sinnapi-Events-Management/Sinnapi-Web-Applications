...
export const metadata: Metadata = {
  title: "How it works",
  description: "Discover vendors, request quotes, book securely with escrow, and manage your event — all on Sinnapi.",
...
function Steps({ title, steps }: { title: string; steps: string[][] }) {
  return (
    <>
      <Typography variant="h3" sx={{ mt: 4, mb: 2 }}>{title}</Typography>
      <Grid container spacing={3}>
        {steps.map(([t, b], i) => (
          <Grid item xs={12} sm={6} md={3} key={t}>
            <Paper variant="outlined" sx={{ p: 3, height: "100%" }}>
              <Typography variant="overline" color="primary">Step {i + 1}</Typography>
              <Typography variant="h6">{t}</Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>{b}</Typography>
            </Paper>
          </Grid>
        ))}
      </Grid>
...
export default function HowItWorksPage() {
  return (
    <>
      <PageHeader title="How Sinnapi works" subtitle="Simple, safe, and built on trust — for clients and vendors alike." />
      <Container sx={{ py: 4 }}>

...
export default async function CategoryPage({ params }: { params: { category: string } }) {
  ...
  return (
    <>
      <PageHeader title={`${name} vendors`} subtitle={`Verified ${name.toLowerCase()} providers for your event.`} />
      <Container sx={{ py: 4 }}>
        <VendorFilterBar defaults={{ category: params.category }} />
        {vendors.length === 0 ? (
          <EmptyState title={`No ${name.toLowerCase()} vendors yet`} ctaLabel="Browse all vendors" ctaHref="/vendors" />
        ) : (

...
function ReviewsList({ vendorId }: { vendorId: string }) {
  ...
  return (
    <QueryState isLoading={isLoading} error={error}>
      {rows.length === 0 ? (
        <EmptyState title="No reviews yet" description="Reviews from completed bookings appear here. You can respond to each one." />
      ) : (
        <Stack spacing={2}>
          {rows.map((r: any) => {
            const response = one<any>(r.review_responses);
            return (
              <Card key={r.id} variant="outlined">
                <CardContent>
                  <Stack direction="row" justifyContent="space-between" alignItems="center">
                    <Typography variant="subtitle1">{one<any>(r.profiles)?.full_name ?? "Client"}</Typography>
                  ...
                  <Box sx={{ mt: 0.5 }}><Typography variant="caption" color="text.secondary">{formatDate(r.created_at)}</Typography></Box>
                  <ReviewResponse reviewId={r.id} existing={response?.body} />
                </CardContent>
              </Card>
            );
          })}

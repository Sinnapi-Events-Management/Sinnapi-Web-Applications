...
type FacetSelectProps = {
  ...
  /**
   * How many events each option would yield under the *other* active filters.
   * Omit for facets the server doesn't count (budget) and the options render
   * plain and fully enabled.
   */
 ...
 *
 * The counts are the difference between a filter bar a vendor explores and one
 * that keeps dead-ending them in an empty feed: an option that would return
 * nothing says so and can't be picked. The currently-selected option is never
 * disabled, even at zero — locking a vendor out of the control they'd need to
 * undo their own selection is worse than showing an empty feed.
 */

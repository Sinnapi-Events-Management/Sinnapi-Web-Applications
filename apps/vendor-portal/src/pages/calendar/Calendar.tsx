...
function CalendarManager({ vendorId }: { vendorId: string }) {
  const { blocked, rows, date, setDate, reason, setReason, busy, err, block, unblock } =
    useCalendar(vendorId);
                ...
                <List>
                  {rows.map((b) => (
                    <ListItem
                      ...
                      secondaryAction={
                        b.source === 'manual' ? (
                          <IconButton edge="end" aria-label="Remove" onClick={() => unblock(b.id)}>
                            <DeleteIcon />
                          </IconButton>
                        ) : (
                    ...
                    >
                      <ListItemText
                        primary={formatDate(b.blocked_date)}
                        secondary={b.reason ?? undefined}
                      />
                    </ListItem>
                  ))}
                </List>

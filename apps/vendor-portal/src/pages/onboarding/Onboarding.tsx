...
export default function Onboarding() {
  const { vendor, app } = useOnboarding();
      ...
      <QueryState isLoading={app.isLoading} error={app.error}>
        {vendor ? (
          <Alert severity="success" sx={{ mb: 2 }}>
            Your business is approved and listed. Head to your <strong>Dashboard</strong> to manage
            bookings.
          </Alert>
        ) : !app.data ? (
          <Card variant="outlined">
            <CardContent>
              ...
              <Typography color="text.secondary" sx={{ mt: 1, mb: 2 }}>
                The full multi-step application (business details, media, verification documents,
                banking, references, and terms) is completed on the public site’s vendor application
                flow.
              </Typography>
              <Button variant="contained" href={`${APP.publicUrl}/apply`}>
                Open application
              </Button>
            </CardContent>
          </Card>
            ...
            <CardContent>
              ...
              {app.data.status === 'rejected' ? (
                <Alert severity="error">
                  Your application was not approved. {app.data.rejection_reason ?? ''} You may
                  re-apply from the public site.
                </Alert>
              ) : (

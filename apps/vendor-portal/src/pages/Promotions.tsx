...
import { Grid, Card, CardContent, Typography, Button, Stack, Chip, Dialog, DialogTitle, DialogContent, DialogActions, TextField, Alert } from "@mui/material";
  ...
  return (
    <>
      <Stack direction="row" justifyContent="flex-end" sx={{ mb: 2 }}>
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => setOpen(true)}>New promotion</Button>
      </Stack>
      <QueryState isLoading={isLoading} error={error}>
        {rows.length === 0 ? (
          <EmptyState title="No promotions" description="Run a promotion to attract more clients." />
        ) : (
              ...
              <Grid item xs={12} sm={6} md={4} key={p.id}>
                <Card variant="outlined" sx={{ height: "100%" }}>
                  <CardContent>
                    ...
                    <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 1 }}>{formatDate(p.starts_at)} – {formatDate(p.ends_at)}</Typography>
                  </CardContent>
                </Card>
              </Grid>
            ))}

...
import { Stack, TextField, Button, Alert, Snackbar, Typography } from "@mui/material";
...
export default function BankAccountForm({ vendorId }: { vendorId: string }) {
  ...
  return (
    <Stack component="form" spacing={2.5} onSubmit={save} sx={{ maxWidth: 480 }}>
      ...
      <Button type="submit" variant="contained" disabled={busy} sx={{ alignSelf: "flex-start" }}>{busy ? "Saving…" : "Save bank account"}</Button>
      <Snackbar open={toast} autoHideDuration={3000} onClose={() => setToast(false)} message="Bank account saved (pending verification)" />
    </Stack>
  );
}

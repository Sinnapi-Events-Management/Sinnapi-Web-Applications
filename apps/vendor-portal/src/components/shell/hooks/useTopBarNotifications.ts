 ...
 *
 * Opening a row stamps it read and navigates to whatever the notification is
 * *about*: a payout lands on `/payouts`, an application on the profile. A bell
 * that only ever drops you on `/notifications` makes you find the same row
 * twice.
 */

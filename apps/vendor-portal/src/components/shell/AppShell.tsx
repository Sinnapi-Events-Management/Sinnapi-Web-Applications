...
export default function AppShell() {
        ...
        {PORTAL_NAV.map(({ label, to, icon: Icon }) => {
          ...
          return (
            <ListItemButton
              ...
              sx={{ mb: 0.25 }}
            >
              <ListItemIcon sx={{ minWidth: 38, color: active ? 'primary.main' : undefined }}>
                <Icon fontSize="small" />
              </ListItemIcon>
  ...
  if (!loading && !vendor) {
    ...
    banner = (
      ...
      >
        Your trial/billing period has ended. You're in the grace period — choose a plan to stay
        visible.
      </Alert>
    );
  }
    ...
    <Box sx={{ display: 'flex', minHeight: '100dvh', bgcolor: 'background.default' }}>
      ...
      >
        <Toolbar>
          ...
          >
            <Avatar
              src={vendor?.primary_image_url ?? profile?.avatar_url ?? undefined}
          ...
          <Menu anchorEl={menuEl} open={!!menuEl} onClose={() => setMenuEl(null)}>
            <MenuItem disabled sx={{ opacity: '1 !important' }}>
              <Box>
                <Typography variant="subtitle2">{vendor?.business_name ?? name}</Typography>
            ...
            <MenuItem component={RouterLink} to="/subscription" onClick={() => setMenuEl(null)}>
              Subscription
            </MenuItem>

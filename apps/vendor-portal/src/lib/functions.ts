 ...
 * BROWSER gives up on it, rather than the vendor.
 *
 ...
 *
 * Mirrors the admin portal's helper — the endpoints and their error shapes are
 * shared, so the unwrapping should not be reinvented per portal.
 */
export async function invokeFunction<T = unknown>(
  ...
  // arrived. Named here so callers can show something a vendor can act on.
  if (error.name === 'FunctionsFetchError') return { data: null, error: 'request_timed_out' };

...
const VENDOR_CARD = "id,slug,business_name,base_city,primary_image_url,profile_image_url,starting_price,starting_price_currency,avg_rating,review_count,is_featured";
...
export function useVendors(q?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      let query = supabase.from("vendors").select(VENDOR_CARD)
        .eq("status", "active").eq("visibility", "public").is("deleted_at", null)
        .order("is_featured", { ascending: false }).order("avg_rating", { ascending: false }).limit(36);
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("vendors")
        ...
        .eq("slug", slug).eq("status", "active").eq("visibility", "public").is("deleted_at", null)
        .maybeSingle();
      if (error) throw error;
      return data;
    },
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("bookings")
        .select("id,reference_no,status,event_date,amount,currency,vendor_id,vendors(business_name,slug,primary_image_url)")
...
export function useBooking(id: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("bookings")
        .select("*,vendors(business_name,slug,primary_image_url)").eq("id", id).maybeSingle();
      if (error) throw error;
      return data;
    },
  });
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("quotations")
        .select("id,reference_no,status,total,currency,valid_until,created_at,vendor_id,vendors(business_name,slug)")
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("events")
        ...
        .eq("source", "client").order("created_at", { ascending: false });
      if (error) throw error;
      return data ?? [];
    },
  });
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("escrow_transactions")
        .select("id,status,gross_amount,net_payout_amount,currency,booking_id,bookings(reference_no),vendors(business_name)")
        .order("created_at", { ascending: false });
      if (error) throw error;
      return data ?? [];
    },
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const [bookings, quotes, escrow, unread] = await Promise.all([
        supabase.from("bookings").select("id", { count: "exact", head: true }).in("status", ["requested", "confirmed", "in_progress"]),
        supabase.from("quotations").select("id", { count: "exact", head: true }).eq("status", "sent"),
        supabase.from("escrow_transactions").select("id", { count: "exact", head: true }).in("status", ["held", "release_requested"]),

...
export default function EventForm() {
  ...
  async function save(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!user) { setBusy(false); setError("Session expired."); return; }
    const { error } = await supabase.from("events").insert({
      posted_by: user.id, source: "client",
  ...
  return (
    <Stack component="form" spacing={2.5} onSubmit={save} sx={{ maxWidth: 560 }}>
      ...
      <TextField name="event_date" type="date" label="Event date" InputLabelProps={{ shrink: true }} />
      <TextField name="location" label="Location" />
      <TextField name="description" label="Description" multiline minRows={4} />
      <Button type="submit" variant="contained" disabled={busy} sx={{ alignSelf: "flex-start" }}>{busy ? "Posting…" : "Post event"}</Button>
    </Stack>
  );
}

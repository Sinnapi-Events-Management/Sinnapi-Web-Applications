 ...
 *
 * WHY THE MESSAGE NAMES NOBODY
 * It used to read "Cannot exceed the {limit}% your vendor proposed." That was
 * false twice over on most bookings: the limit came from the platform's own
 * `advance_rate_max`, not from the vendor, and on any booking without a
 * quotation behind it the vendor had proposed nothing whatsoever. Attributing
 * a platform limit to the other party — on the one screen where the client is
 * agreeing to money leaving their account before they get anything — is the
 * kind of wrong that makes a person distrust the rest of the page.
 *
 * The limit is Sinnapi's, so the sentence says Sinnapi. What the vendor asked
 * for is a separate fact and belongs where it is true: beside the control, as a
 * suggestion.
 */
      ...
      .superRefine((v, ctx) => {
        ...
        if (Number(v) > limit) {
          ctx.addIssue({
            ...
            message: `Sinnapi releases at most ${limit}% of a booking before the event.`,
          });
        }
      }),
  });

...
import {
  ...
  IconButton, Divider, Avatar, Menu, MenuItem, Badge, Tooltip,
} from "@mui/material";
  ...
  const nav = (
    <Box sx={{ px: 1.5, py: 2 }}>
      <Typography component={RouterLink} to="/dashboard" variant="h5"
        sx={{ display: "block", px: 1.5, mb: 2, fontFamily: '"Fraunces", serif', fontWeight: 600, color: "primary.main", textDecoration: "none" }}>
        {APP.name}
      </Typography>
      <List>
        {CLIENT_NAV.map(({ label, to, icon: Icon }) => {
          ...
          return (
            <ListItemButton key={to} component={NavLink} to={to} selected={active} onClick={() => setMobileOpen(false)} sx={{ mb: 0.5 }}>
              <ListItemIcon sx={{ minWidth: 40, color: active ? "primary.main" : undefined }}><Icon /></ListItemIcon>
      ...
      <List>
        {ACCOUNT_NAV.map(({ label, to, icon: Icon }) => (
          <ListItemButton key={to} component={NavLink} to={to} selected={pathname.startsWith(to)} onClick={() => setMobileOpen(false)} sx={{ mb: 0.5 }}>
        ...
        <Toolbar>
          ...
          <Menu anchorEl={menuEl} open={!!menuEl} onClose={() => setMenuEl(null)}>
            <MenuItem disabled sx={{ opacity: "1 !important" }}>
              <Box><Typography variant="subtitle2">{name}</Typography><Typography variant="caption" color="text.secondary">{email}</Typography></Box>
            </MenuItem>
            <Divider />
            <MenuItem component={RouterLink} to="/profile" onClick={() => setMenuEl(null)}>Profile</MenuItem>

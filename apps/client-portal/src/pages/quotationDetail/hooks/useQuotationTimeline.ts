 ...
 * happened half is authoritative rather than inferred — including the vendor's
 * own moves, which the quotation row itself does not record, and the reason
 * given on a void or a revision request. The projected half exists so a quote
 * waiting on someone else's action shows what that action is; it disappears
 * once the quote is accepted, declined, expired or voided, where there is no
 * "next" to point at.
 */

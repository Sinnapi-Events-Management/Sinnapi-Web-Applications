...
type Options = {
  /** The rate the quote was priced at — the vendor's proposal on first load. */
  proposedRate: number | null;
  /** The most the client may choose, from the same quote. */
  limit: number | null;
};
 ...
 */
export function useAdvanceRate({ proposedRate, limit }: Options) {
  ...
  // The field mounts before the quote arrives, so it starts blank and adopts
  // the vendor's proposal once priced — clamped, because a quotation drafted
  // before an admin tightened the ceiling can propose more than is now
  // allowed. Seeded once: re-seeding would overwrite what the client chose.
  const seeded = useRef(false);
  useEffect(() => {
    if (seeded.current || proposedRate == null || limit == null) return;
    seeded.current = true;
    reset({ advance_rate: String(Math.min(proposedRate, limit)) });
  }, [proposedRate, limit, reset]);
  ...
  return {
    ...
    /** The client has moved off the maximum their vendor agreed to. */
    isCustomised: parsed != null && limit != null && parsed < limit,

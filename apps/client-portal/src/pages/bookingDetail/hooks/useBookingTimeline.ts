 ...
 * happened half is authoritative rather than inferred — including the moves the
 * vendor made, which the booking row itself does not record. The projected half
 * exists so a booking waiting on someone else's action shows what that action
 * is; it disappears once the booking is cancelled or declined, where there is no
 * "next" to point at.
 */

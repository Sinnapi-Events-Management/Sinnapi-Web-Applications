...
export function usePaymentReturn() {
  ...
  if (!ret) {
    ...
    // The payer backed out of the provider's hosted checkout. If there is
    // no row at all (cancel URL without a pid, or the payment row is not
    // visible through RLS), say so honestly.
    state = 'cancelled';
    ...
    // The payer backed out before the provider captured anything.
    state = 'cancelled';
  ...
  // The page header's one-line summary. Derived here beside the state it
  // describes, so a new state cannot be added without answering what the
  // header says about it — the ternary chain this replaces lived in the page
  // and silently fell through to "Checking on your payment."
  const subtitle =
    state === 'cancelled'

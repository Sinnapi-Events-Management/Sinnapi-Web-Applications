...
// The admin portal has the same parser at `packages/utils/userAgent.ts`, and
// the duplication is a runtime boundary rather than an oversight: Edge
// Functions are bundled from `supabase/functions/` alone and cannot import from
// the pnpm workspace, and the browser cannot import a Deno module. The codebase
// already carries this kind of pair deliberately — `client-sign-up`'s EMAIL_RE
// mirrors the client-portal zod schema for the same reason.
//
// The two must stay in step. They are the same logic, in the same order, with
// the same tables; a change to one is a change to both, and the tables are
// small precisely so that stays cheap.
//
// WHY NOT `ua-parser-js`
// This answers one question — "is this the device the account normally signs in
// from?" — for which "Chrome on Windows" is as useful as an exhaustively
// versioned parse, and it answers it on the critical path of every sign-in. A
// parser library is a dependency that needs updating to keep recognising new
// browsers, for accuracy nobody here consumes.
//
...
export type ParsedUserAgent = {
  /** e.g. "Chrome 141" */
  browser: string | null;
  /** e.g. "Windows 10/11", "iOS 17" */
  os: string | null;
  /** Rough form factor. Clamped to this set by `_clean_device` in 0802e. */

...
const PROFANITY = (Deno.env.get('PROFANITY_LIST') ?? '')
  .split(',')
...
function score(text: string): {
  ...
  if (PROFANITY.some((w) => w && lower.includes(w)))
    return { status: 'blocked', score: 1, reason: 'profanity' };
...
Deno.serve(
  handler(async (req) => {
    // Server-to-server only. The gateway does not verify a JWT here, and the
    // body names the row to update by id, so an open endpoint would let anyone
    // overwrite the moderation verdict on any message or review with the
    // service-role client. Only a trusted caller holding our own key may.
    ...
    if (kind === 'message') {
      await supa
        .from('messages')
        .update({ moderation_status: verdict.status, moderation_score: verdict.score })
        .eq('id', id);
      if (verdict.status !== 'clean') {
        await supa.from('message_flags').insert({
          message_id: id,
          flagged_by: null,
          reason: verdict.reason ?? 'other',
          status: 'open',
        });
      }
    } else if (kind === 'review') {
      await supa
        .from('reviews')
        .update({ moderation_status: verdict.status === 'clean' ? 'clean' : 'flagged' })
        .eq('id', id);
      if (verdict.status !== 'clean') {
        await supa.from('review_reports').insert({
          review_id: id,
          reported_by: null,
          reason: verdict.reason ?? 'other',
          status: 'open',

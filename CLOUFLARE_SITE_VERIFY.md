Wire up server-side siteverify for my existing Turnstile widget in this project. Don't change the site key. Embed the widget on the right forms, add the canonical siteverify call in the backend, and validate.

Widget: Sinnapi (0x4AAAAAAEEkwoJxJ_KENQZb)

Fetch the widget's secret via the Cloudflare API using your CLOUDFLARE_API_TOKEN — see the secret-retrieval decision tree in the inlined skill. Don't ask me to paste the secret unless the API path fails.

Constraints:

- Don't create a new widget; the one above already exists.
- Store the secret as TURNSTILE_SECRET in the environment, never inline.
- Use canonical siteverify (POST to challenges.cloudflare.com/turnstile/v0/siteverify). Don't deploy any extra infrastructure.

This is a recovery flow against an existing widget — skip Step 8 (widget creation) in the wizard below and use the sitekey above instead.

The full Turnstile Spin skill is inlined below so you can start immediately without fetching anything. Reference URL (only fetch if you need an updated copy): https://developers.cloudflare.com/turnstile/spin/?widget=0x4AAAAAAEEkwoJxJ_KENQZb

...
/**
 * Airtel Money, as Airtel's red swirl.
 *
 * Airtel publishes no Airtel Money artwork, so this is the parent Airtel mark
 * (geometry from Simple Icons, CC0) in Airtel red. The rail's own label names
 * the product, so the mark only has to be recognisably Airtel. Replace this
 * body with the official file if one arrives through the Pesapal merchant
 * account; `ProviderLogo` is the only thing that imports it.
 */
export function AirtelMoneyMark({ title }: { title: string }) {
  return (
    <svg viewBox="-1 -1 26 26" role="img" aria-label={title} width="100%" height="100%">
      <path
        fill={brand.airtel.red}
